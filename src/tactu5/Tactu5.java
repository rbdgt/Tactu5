...
/**
 * Tactu5 class allows users to interact with internal sequencer.
 * 
 * 
 * 
 * 
...
public class Tactu5 implements Tactu5ConstantsPack {
	...
	public Tactu5(PApplet p, InternalSequence s, boolean b) {
		...
		/*
		 * try { fancyEventMethod =
		 * parent.getClass().getMethod("fancyEventMethod", new Class[] {
		 * String.class }); } catch (Exception e) { // no such method, or an
		 * error.. which is fine, just ignore }
		 */
		// tactusLoopFinish ();
	}

	/*
	 * public void makeEvent() { if (fancyEventMethod != null) { try {
	 * fancyEventMethod.invoke(parent, new Object[] { String.class }); } catch
	 * (Exception e) { System.err.println("Disabling fancyEvent() for " + name +
	 * " because of an error."); e.printStackTrace(); fancyEventMethod = null; }
	 * } }
	 */
	...
	/**
	 * Rewind sequencer to initial loop position.
	 * 
	 */
	public void rewind() {
	...
	/**
	 * Rewind and stop sequencer to initial loop position
	 * 
	 * 
	 */
	...
	// sequencer check controller class for updating content to be played

	/**
	 * This method allows dynamically changing Tactu5 internal sequencer through
	 * Aggregator output.
	 * 
	 * @param s InternalSequence
	 * 
	 */
