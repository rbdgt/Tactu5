...
public class Tactu5SimpleSequencer extends Thread {
	...
	public Tactu5SimpleSequencer(PApplet p, InternalSequence s, Tactu5Controller o, boolean b) {
		...
		try {
			fancyEventMethod = parent.getClass().getMethod("noteReceiver", new Class[] { Note.class });

		} catch (Exception e) {
			System.out.println("ALERT noteReceiver(Note) method is not set!");
		}
		...
		try {
			...
			System.out.println("ALERT loop method is not set!");
		}
				...
				if (oldT > timeline.getTotalTime()) {
					...
					for (int ct = 0; ct < timeline.getClusterNumber(); ct++) {
						...
						if (newTimeAt == oldT) {
							...
							// make a rest to fit to new rythmic pattern
